...
Indichiamo i quattro autostati di $\hat{\vec{S}}^{\,2}$ e $\hat{S}_z$ con $\Ket{s, m}$, e questi sono
\[
\Ket{1, -1},\quad \Ket{1, 0},\quad \Ket{1, 1},\quad \Ket{0, 0}
\]

% Gli autostati che formano la base dello spazio ottenuto dal prodotto tensoriale dei due spazi
La base di autostati che si ottengono dal prodotto tensoriale degli autostati di $\hat{\vec{S}}^{\,2}_1$ e $\hat{S}_{1z}$ o $\hat{\vec{S}}_2^{\,2}$ e $\hat{S}_{2z}$ sono
...
\end{equation*}
% dove ciascun ket $\Ket{s_i, m_i}$ rappresenta uno degli autostati di $\hat{\vec{S}}^{\,2}_1$ e $\hat{S}_{1z}$ o $\hat{\vec{S}}_2^{\,2}$ e $\hat{S}_{2z}$.
...
Un'altra proprietà utile che mette in relazione il numero quantico $m$ di spin totale con i due numeri $m_1$ e $m_2$ degli spin singoli è
\[
    m = m_1 + m_2
\]
...
Siccome i fattori di fase non hanno alcuna rilevanza dal punto di vista fisico (e quindi anche un semplice $-1$) possiamo scegliere a piacere
