...
\begin{homeworkProblem}
    ...
    Uso ancora integrazione per parti (ma al contrario) e trascuro integrale di superficie
    ...
    \end{align*}
Applicando nuovamente la formula (...) si nota che dal secondo commutatore si ottengono due commutatori che danno entrambi 0.
Applichiamo (...) anche al primo, trovando
\begin{align*}
    &\frac{1}{2} \sum_{\sigma', \sigma''} \int_\Omega d^3r' d^3r'' v(\pvec{r}' - \pvec{r}'') \hat{\psi}_{\sigma'}^\dagger(\pvec{r}')\hat{\psi}_{\sigma''}^\dagger(\pvec{r}'')\left[\hat{\psi}_{\sigma''}(\pvec{r}'')\hat{\psi}_{\sigma'}(\pvec{r}'), \hat{\psi}_\sigma^\dagger(\vec{r}, t)\right]
    \\
...
\end{align*}
    ...
    Sfruttiamo $\left[\hat{\psi}_{\sigma}^\dagger(\vec{r}), \hat{\psi}_{\sigma'}^\dagger(\pvec{r}')\right] = 0$ e $v(\vec{r} - \pvec{r}') = v(\pvec{r}' - \vec{r})$
    ...
    Possiamo usare la formula (...), e siccome $\vec{k} = \pvec{k}'$ e $\sigma = \sigma'$ troviamo
    \begin{equation*}
    ...
    Dato che  nel secondo termine $\hat{c}_{\vec{k}, \uparrow}$ viene applicato a $\Ket{0}$ prima di $\hat{c}_{\vec{k}, \uparrow}^\dagger$, significa che stiamo cercando di distruggere una particella con vettore d'onda $\vec{k}$ e spin $\uparrow$ prima che questo venga creato,
    quindi questo termina darà un contributo nullo.
